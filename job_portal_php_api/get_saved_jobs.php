...
$js_id = $_GET['js_id'];
	...
	$saved_jobs = mysql_result(mysql_query("select saved_jobs from jobseeker where jobseeker_id=$js_id"),0);
//     echo $saved_jobs;
//      echo '<br/>';
      $itemCount =  substr_count($saved_jobs, ",");
//      echo $itemCount;

if(!empty($saved_jobs)){
      if($itemCount>0){
            $q = "select  distinct * from jobs,employers where jobs.job_id in ($saved_jobs) and jobs.employer_id=employers.emp_id  order by job_id asc limit ". (($page-1) * $item_per_page ) .",".$item_per_page;
      }else{
            $q = "select * from jobs,employers where jobs.job_id=$saved_jobs and jobs.employer_id=employers.emp_id limit ". (($page-1) * $item_per_page ) .",".$item_per_page;
      }
