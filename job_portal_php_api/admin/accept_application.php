      ...
      if(isset($_GET['submit']) && $_GET['submit']==1){
            ...
            if(empty($message)){
                  ...
                  $q = "insert into job_acceptance(job_id,js_id,status,message)
                        values({$job_id}, {$js_id}, 1, '{$message}')";
                  }else{
                  $q = "update job_acceptance set message='{$message}', status=1
...
?>
    ...
    <div class="container">
		<h2 style="color:#999; margin-top:40px;">Accept Application</h2>
...
if($status=="" || $override==1){ 
       ...
       echo '
		<form role=form method="get" action="accept_application.php">';
      ...
      echo (!empty($msgErr))? '<p class="failure">'.$msgErr.'</p>':"";
      
       echo'<textarea class="form-control" cols="50" rows="6" name="message"></textarea><br/>
      <button type="submit" name="submit" value="1" class="btn btn-success">Accept Application</button>
