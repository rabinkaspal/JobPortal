...
class DB_Functions {
	...
	public function storeUser2($js_id, $gcm_regid) {
        // insert user into database
        $result = mysql_query("INSERT INTO notification(js_id, regId) VALUES('$js_id','$gcm_regid')");
        // check for successful store
        if ($result) {
            // get user details
            $id = mysql_insert_id(); // last inserted id
            $result = mysql_query("SELECT * FROM notification WHERE id = $id") or die(mysql_error());
            // return user details
            if (mysql_num_rows($result) > 0) {
                return mysql_fetch_array($result);
            } else {
